...
class Transformacje:
    def __init__(self, model: str = "wgs84"):
        ...
        self.ecc = m.sqrt(2 * self.flat - self.flat ** 2) # eccentricity  WGS84:0.0818191910428 
    ...
    def xyz2plh(self, X, Y, Z, output = 'dec_degree'):
        ...
        lat_prev = m.atan(Z / (r * (1 - self.ecc2)))    # pierwsze przybliilizenie
        lat = 0
        while abs(lat_prev - lat) > 0.000001/206265:    
            lat_prev = lat
            N = self.a / m.sqrt(1 - self.ecc2 * m.sin(lat_prev)**2)
            h = r / m.cos(lat_prev) - N
            lat = m.atan((Z/r) * (((1 - self.ecc2 * N/(N + h))**(-1))))
        lon = m.atan(Y/X)
        N = self.a / m.sqrt(1 - self.ecc2 * (m.sin(lat))**2);
        h = r / m.cos(lat) - N       
        ...
        def GK_2_blh(self, xGK, yGK, m, lam0):    
            """   
            Funkcja przelicza współrzędne Gaussa - Krugera  
            na współrzędne geodezyjne.
            ...
            N = self.a/m.sqrt(1 - self.e2 * m.sin(b)**2)
            t = m.tan(b)
            n2 = e_2 * m.cos(b)**2
        ...
        def u92u00_2_blh(self, X, Y):
            ...
            """     
            xGK, yGK, lam0, m = u92u00_2_GK(X, Y)
            fi, lam = GK_2_blh(xGK, yGK, m, lam0, self.e2, self.a)
        ...
        def sel_sUK_red(self, X1, X2, s):
            ...
            """     
            fip, lamp = u92u00_2_blh(X1[0], X1[1], self.e2, self.a)
            fik, lamk = u92u00_2_blh(X2[0], X2[1], self.e2, self.a)
            
            xGKp, yGKp, lam0p, m = u92u00_2_GK(X1[0], X1[1])
            xGKk, yGKk, lam0k, m = u92u00_2_GK(X2[0], X2[1])
           
            fim = (fik + fip)/2
               
            R, N, M = R_M_N(self.a, self.e2, fim)
        ...
        def sel_az_vincent(self, fl1, fl2):
            """   
            Funkcja przelicza współrzędne Gaussa - Krugera  
            na współrzędne geodezyjne.
        ...
        def R_M_N(self, fi):
            ...
            """     
            N = self.a/m.sqrt(1 - self.e2 * m.sin(fi)**2)
            M = self.a * (1 - self.e2)/m.sqrt((1 - self.e2 * m.sin(fi)**2)**3)
            R = m.sqrt(M * N)
